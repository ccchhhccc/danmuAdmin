<template>
	<div class="right">
		...
		<!--频道表格-->
		<Table :columns="columns1" :data="recommend"></Table>
		...
		<Modal v-model="modal3" width="360">
	        ...
	        <div slot="footer" style="text-align:center">
	            ...
	            <Button type="error" @click="toDelRecommend">删除</Button>
	        </div>
	    </Modal>
	</div>
</template>
	...
	export default {
		...
		methods:{
			...
			toDelRecommend(){
				...
				$.ajax({
					type:"post",
					url:"http://localhost:2255/recommend/del",
					data:{
						t_id:that.delid
					},
					async:false,
					success:function(data){
						that.$Message.success('删除推荐成功')
						that.closeDel()
			...
			toUpdateSort(){
				...
				$.ajax({
					type:"post",
					url:"http://localhost:2255/recommend/update/sort",
			...
			getDetailById(id){
				...
				$.ajax({
					type:"post",
					url:"http://localhost:2255/recommend/id",
					data:{
						t_id:id
					},
					async:false,
